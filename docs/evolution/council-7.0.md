...
【開場】7.0 週期診斷：GTX 自動 14/22，使用者模擬 10/15。
GA 門檻：GTX≥78、使用者模擬≥8。
P0 阻塞：無。
...
【開場】下一版 8.0（hermes-247）焦點：Hermes VPS worker 常駐、Cloud-First 為預設路徑。
...
定稿：本週 P0 = 修復 #11 搜尋不寫記憶 → 更新 docs/evolution/7.0-cycle-report.md → 準備 8.0 wave 交付。
...
【回應】安全審計員提醒的 HITL 可放到 7.0，不阻塞 8.0 GA。
...
3. **P0 行動**：
   1. 修復 #11 搜尋不寫記憶
   2. 更新 docs/evolution/7.0-cycle-report.md
   3. 準備 8.0 wave 交付
4. **下一版 8.0（hermes-247）**：Hermes VPS worker 常駐；Cloud-First 為預設路徑；Cursor worker 自動恢復。
